...
from mcp.types import TextContent, CallToolRequest, ListToolsRequest, CallToolRequestParams
        ...
        with patch("os.remove"):
            with patch("builtins.open", MagicMock(side_effect=[
                ...
                req = types.CallToolRequest(
                    ...
                    params=types.CallToolRequestParams(
                        name="read_memory",
                        arguments={"node_id": node_id, "address": 0x1000, "size": 4}
                    )
                )
                res = await handler(req)
...
async def test_call_tool_send_uart(server):
    ...
    req = types.CallToolRequest(
        ...
        params=types.CallToolRequestParams(
            ...
            arguments={"node_id": node_id, "data": "hello"}
        )
    )
    res = await handler(req)
...
async def test_read_resource_console(server):
    ...
    req = types.ReadResourceRequest(
        ...
        params=types.ReadResourceRequestParams(uri=f"virtmcu://nodes/{node_id}/console")
    )
    res = await handler(req)

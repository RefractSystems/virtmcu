...
from tools.repl2qemu.parser import ReplDevice, ReplPlatform, ReplInterrupt
from tools.yaml2qemu import parse_yaml_platform, validate_dtb, main
from tools.qmp_probe import QMPClient, dump_tree, main as qmp_main
from tools.repl2yaml import migrate, main as repl2yaml_main
...
def test_repl2qemu_main_basic(tmp_path):
    ...
    test_args = ["repl2qemu.py", str(repl_file), "--out-dtb", str(dtb_file), "--out-arch", str(arch_file), "--print-cmd"]
    
    with patch("sys.argv", test_args), \
         patch("tools.repl2qemu.__main__.compile_dtb", return_value=True):
        repl2qemu_main()
...
def test_qmp_main_tree():
    with patch("tools.qmp_probe.QMPClient") as mock_client_cls, \
         patch("tools.qmp_probe.dump_tree") as mock_dump_tree:

...
def get_rust_binary_path(name: VirtmcuBinary | str) -> Path:
    ...
    if isinstance(name, VirtmcuBinary):
        ...
        try:
            ...
            warnings.warn(
                f"Usage of hardcoded binary string '{name}' is deprecated. "
                f"Use VirtmcuBinary.{matched.name} instead.",
                DeprecationWarning,
                stacklevel=2,
            )
...
def resolve_rust_binary(name: VirtmcuBinary | str) -> Path:
    ...
    if not p.exists():
        ...
        raise FileNotFoundError(
            f"Binary {msg_name} not found. Searched path: {p}. Did you run 'cargo build'?"
        )
    return p

...
wait_for_queryable() {
    ...
    while (( $(date +%s) < deadline )); do
        if python3 -c "import zenoh, sys, struct; s=zenoh.open(); r=list(s.get('$topic', payload=struct.pack('<QQ', 0, 0), timeout=0.5)); s.close(); sys.exit(0 if r else 1)" 2>/dev/null; then
            return 0
        fi
        sleep 0.25
    done

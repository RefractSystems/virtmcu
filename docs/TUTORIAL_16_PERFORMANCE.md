...
Phase 16 introduces systematic benchmarking so you can measure the **actual overhead** of the Zenoh-based clock synchronization and detect performance regressions automatically in CI.
...
```
  ...
  │                                                                │
  │   1. Compiles minimal.dtb from test/phase1/minimal.dts         │
...
```bash
cd /workspace
bash test/phase16/smoke_test.sh
```
...
2. **Shared CI runner**: absolute MIPS thresholds assume a dedicated host. On a shared runner, add `--shared-ci` to enable trend-based comparison instead.
...
## 6. Performance Trend Tracking (Phase 16.5)

After each benchmark run, results are saved to `test/phase16/last_results.json`. The `scripts/perf_trend.py` tool compares against the established baseline:

```bash
# Establish baseline (first run)
python3 scripts/perf_trend.py --save-baseline

# Check for regression (subsequent runs)
python3 scripts/perf_trend.py --check
```

The check fails if:
- MIPS regresses > 5% vs. baseline
- P99 latency increases > 10% vs. baseline

To update the baseline after a justified architectural change:
```bash
python3 scripts/perf_trend.py --save-baseline
git add test/phase16/baseline.json
git commit -m "perf: update baseline after <reason>"
```

---

## 7. Jitter Injection Test (Phase 16.4)
...
2. Running 10 independent `slaved-icount` benchmark runs through the proxy.
3. Asserting all 10 `exit_vtime_ns` values are identical.

```bash
bash test/phase16/jitter_test.sh
```
...
1. Run the baseline benchmark: `python3 test/phase16/bench.py`
2. Modify `test/phase16/bench.py` to add `-device zenoh-telemetry,node=0,router=<router>` to the QEMU command for `slaved-icount` mode.

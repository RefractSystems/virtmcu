...
Declaring a subscriber in Zenoh is an asynchronous operation. To prevent races where the first virtual-time packet is dropped because the router has not yet fully propagated the declaration, VirtMCU provides the `ensure_session_routing(session)` helper. This tool uses a liveliness-probe roundtrip to verify that the router has fully ingested the session's declaration backlog before emulated code begins to execute.
